...
//  ros::Subscriber sub_incPositionOdom = nh.subscribe ("/incPositionOdom", 1, callback_incPositionOdom);
  ...
  */
  ...
  while(ros::ok()){
    ...
    std::cout << "flag while 1" << std::endl;
    ...
    std::cout << "flag while 2" << std::endl;
    ...
    std::cout << "flag while 3" << std::endl;
    /*
    ...
    */
    ...
    std::cout << "flag while 4" << std::endl;
    ...
    std::cout << "flag while 5" << std::endl;
    pub_ekfIncOdomRPY.publish(incOdomEKF);


    std::cout << "flag while 6" << std::endl;
...
//    positionPredEKF = Comp(positionEKF, incPositionOdom); // -----------------------> Position prediction (from pcl_segment::positionRPY incPositionOdom)
    ...
    std::cout << "flag while 7" << std::endl;
...
//    std::cout << "Fu: " << Fu << std::endl;
    ...
    std::cout << "flag while 8" << std::endl;
...
//    std::cout << "P: " << P << std::endl;
    ...
    MatrixXf h(tamMax, 1); h = h.Zero(tamMax, 1);
    MatrixXf Hx(tamMax, 6); Hx = Hx.Zero(tamMax, 6);
    std::vector<Matrix<float, Dynamic, 6>> Hz_v;
    Vector6f hi; hi = hi.Zero();
    Matrix6f Hxi; Hxi = Hxi.Zero();
    Matrix6f Hzi; Hzi = Hzi.Zero();
    int tam = 0;
  ...
  /**/    hi = computeInnovation(positionPredEKF, observations_BL[i_vec[i]], map[j_vec[i]]);
  ...
  /**/  S = Hx_resized*PHt;
      ...
      else // if there are observations but there are no matches, we stay with predictions
      {
        std::cout << "else position = prediction" << std::endl;
        positionCorrEKF = positionPredEKF;
      }
    } // if there are no observations, we stay with prediction
    else
    {
      std::cout << "else position = prediction 2" << std::endl;
      positionCorrEKF = positionPredEKF;
    }
/*     
*/
