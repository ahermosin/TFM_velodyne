...
{
  //tf::transformStampedMsgToTF(input_incOdom, incOdom);
...
main (int argc, char** argv)
{
  ...
  float sigma_odom_x = 0.5;
  float sigma_odom_y = 0.5;
  float sigma_odom_z = 0.5;
  float sigma_odom_qx = 0.1;
  float sigma_odom_qy = 0.1;
  float sigma_odom_qz = 0.1;
  float sigma_odom_qw = 0.1; // odometry sigmas
  
  float sigma_obs_x = 0.1;
  float sigma_obs_y = 0.1;
  float sigma_obs_z = 0.1;
  float sigma_obs_qx = 0.05;
  float sigma_obs_qy = 0.05;
  float sigma_obs_qz = 0.05;
  float sigma_obs_qw = 0.05; // observation sigmas
  
  int i,j;
  float P[7][7]; // Kalman covariance matrix
  float R[7][7]; // Observation covariance matrix
  float Q[7][7]; // Odometry covariance matrix
  ...
  R[0][0] = sigma_odom_x*sigma_odom_x;
  R[1][1] = sigma_odom_y*sigma_odom_y;
  R[2][2] = sigma_odom_z*sigma_odom_z;
  R[3][3] = sigma_odom_qx*sigma_odom_qx;
  R[4][4] = sigma_odom_qy*sigma_odom_qy;
  R[5][5] = sigma_odom_qz*sigma_odom_qz;
  R[6][6] = sigma_odom_qw*sigma_odom_qz;
  ...
  while(ros::ok()){
    ...
    posePredEKF.pose.position.x = poseEKF.pose.position.x + incOdom.transform.translation.x;
    posePredEKF.pose.position.y = poseEKF.pose.position.y + incOdom.transform.translation.y;
    posePredEKF.pose.position.z = poseEKF.pose.position.z + incOdom.transform.translation.z;
    
    posePredEKF.pose.orientation.x = poseEKF.pose.orientation.x + incOdom.transform.rotation.x;
    posePredEKF.pose.orientation.y = poseEKF.pose.orientation.y + incOdom.transform.rotation.y;
    posePredEKF.pose.orientation.z = poseEKF.pose.orientation.z + incOdom.transform.rotation.z;
    posePredEKF.pose.orientation.w = poseEKF.pose.orientation.w + incOdom.transform.rotation.w;

    quat.setX(posePredEKF.pose.orientation.x);
    quat.setY(posePredEKF.pose.orientation.y);
    quat.setZ(posePredEKF.pose.orientation.z);
    quat.setW(posePredEKF.pose.orientation.w);
    quat.normalize();
    
    posePredEKF.pose.orientation.x = quat.getX();
    posePredEKF.pose.orientation.y = quat.getY();
    posePredEKF.pose.orientation.z = quat.getZ();
    posePredEKF.pose.orientation.w = quat.getW(); // posePredEKF calculated
...
Matrix Kalman_Loc::KalmanPredict(float odom_pose_x, float odom_pose_y, float odom_pose_z, float odom_pose_roll, float odom_pose_pitch, float odom_pose_yaw)//Next kalman position
{
	...
	kalman_pos << Comp(kalman_pos, inc_odom);	//the prediction ------ kalman_pos <-> poseEKF
